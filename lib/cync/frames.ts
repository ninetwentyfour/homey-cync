...
import { FACTORY_EFFECTS } from './effects';
...
function nextOuterCounter(): number {
  const v = outerCounter;
  outerCounter = (outerCounter + 1) % 0x10000;
  return v;
}

function nextInnerCounter(): number {
  const v = innerCounter;
  innerCounter = innerCounter + 1 < 0xfffffffe ? innerCounter + 1 : 257;
  return v;
}

export interface ParsedOuterFrame {
  messageType: number;
  isResponse: boolean;
  version: number;
  payload: Buffer;
  /** Total bytes consumed from the input buffer (including header). */
  consumed: number;
}
...
export function buildLoginPacket(authorize: string, userId: number): Buffer {
  const versionByte = Buffer.from([PROTOCOL_VERSION]);
...
function buildPipePayload(hubDeviceId: number, inner: Buffer): Buffer {
...
export function buildPipePacket(hubDeviceId: number, inner: Buffer): Buffer {
...
export function buildProbePacket(deviceId: number): Buffer {
  const idBytes = Buffer.alloc(4);
...
function escape7e(body: Buffer): Buffer {
  const out: number[] = [];
  for (const b of body) {
    if (b === 0x7e) {
      out.push(0x7d, 0x5e);
...
function unescape7e(body: Buffer): Buffer {
  const out: number[] = [];
 ...
 */
function buildInnerFrame(commandCode: PipeCommand, commandBody: Buffer): Buffer {
  const seq = Buffer.alloc(4);
  seq.writeUInt32LE(nextInnerCounter(), 0);
  const direction = Buffer.from([PipeDirection.REQUEST]);

  const doubleSeq =
    commandCode === PipeCommand.SET_POWER_STATE ||
...
function meshIdLe(meshId: number): Buffer {
...
export function buildQueryDeviceStatusInner(): Buffer {
  const seq = Buffer.alloc(4);
  seq.writeUInt32LE(nextInnerCounter(), 0);
  const direction = Buffer.from([PipeDirection.REQUEST]);
...
export function buildSetPowerInner(meshId: number, isOn: boolean): Buffer {
...
export function buildSetBrightnessInner(meshId: number, brightness: number): Buffer {
...
export function buildSetColorTempInner(meshId: number, tempPct: number): Buffer {
...
export function buildSetRgbInner(meshId: number, r: number, g: number, b: number): Buffer {
  const body = Buffer.concat([
    Buffer.from([0x00]),
    meshIdLe(meshId),
...
export function buildSetEffectInner(meshId: number, effect: EffectName): Buffer {
  const [b1, b2] = FACTORY_EFFECTS[effect];
  const body = Buffer.concat([
    Buffer.from([0x00]),
    meshIdLe(meshId),
...
export function buildComboInner(
  meshId: number,
  isOn: boolean,
  brightness: number,
...
// ── inner-frame parser (for state updates) ──

export interface ParsedStatusDevice {
  meshId: number;
  isOn: boolean;
  brightness: number;
  colorMode: number;
 ...
 */
export function parsePipeStatusPages(payload: Buffer): ParsedStatusDevice[] | null {
  if (payload.length <= 7 || payload[7] !== 0x7e) return null;
  let inner = payload.subarray(7);
  if (inner[0] !== 0x7e || inner[inner.length - 1] !== 0x7e) return null;
  ...
  const out: ParsedStatusDevice[] = [];
  for (let i = 0; i < deviceCount; i++) {
    if (cursor.length < 24) break;
