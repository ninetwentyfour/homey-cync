...
exports.CyncAuth = exports.CyncAuthError = void 0;
const axios_1 = __importDefault(require("axios"));
const CORP_ID = '1007d2ad150c4000';
const BASE_URL = 'https://api.gelighting.com';
class CyncAuthError extends Error {
...
exports.CyncAuthError = CyncAuthError;
class CyncAuth {
    constructor(opts = {}) {
    ...
    async login(email, password) {
        ...
        if (res.status === 400) {
            ...
            throw new CyncAuthError('Two-factor verification required. Check your email for a code.', 'needs_otp');
        }
        if (res.status !== 200) {
        ...
        return this.toStoredTokens(res.data);
    }
    async submitOtp(email, password, code) {
        this.logger.log('Cync auth: submitting OTP');
        const resource = Array.from({ length: 16 }, () => String.fromCharCode(97 + Math.floor(Math.random() * 26))).join('');
    ...
    async post(path, body) {
        try {
            const res = await axios_1.default.post(`${BASE_URL}${path}`, body, {
