    ...
    async listDevices() {
        ...
        for (const home of homes) {
            ...
            if (meshBulbs.length > 0) {
                const first = meshBulbs[0];
                this.logger.log(`home ${home.id}: first bulb keys=${Object.keys(first).join(',')}`);
                this.logger.log(`home ${home.id}: first bulb firmwareVersion=${String(first.firmwareVersion)} deviceType=${String(first.deviceType)} mac=${String(first.mac)} wifiMac=${String(first.wifiMac)}`);
            }
