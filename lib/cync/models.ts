...
export type DeviceClassification = 'light' | 'switch' | 'plug' | 'thermostat' | 'unknown';
...
export const DEVICE_TYPE_MAP: Record<number, ModelSpec> = {
...
export function lookupModel(deviceType: number | undefined): ModelSpec | undefined {
...
export function estimateWattsActive(spec: ModelSpec | undefined): number | undefined {
  if (!spec) return undefined;
  if (spec.wattsActive !== undefined) return spec.wattsActive;
  if (spec.lumens) return Math.max(1, Math.round(spec.lumens / 90));
  return undefined;
}

export function estimateWattsIdle(spec: ModelSpec | undefined): number {
  return spec?.wattsIdle ?? IDLE_DEFAULT;
}

/** "Full Color A19 Bulb (CLEDA199CDRV)" — SKU appended when known. */
export function formatModelName(spec: ModelSpec | undefined): string {
  if (!spec) return '';
  if (spec.modelId) return `${spec.modelName} (${spec.modelId})`;
  return spec.modelName;
}

/** "800 lm · 2000–7000K · 90 CRI" (segments omitted when unknown). */
export function formatSpecsLine(spec: ModelSpec | undefined): string {
  if (!spec) return '';
  const parts: string[] = [];
  if (spec.lumens) parts.push(`${spec.lumens} lm`);
  ...
  if (spec.cri) parts.push(`${spec.cri} CRI`);
  return parts.join(' · ');
}
